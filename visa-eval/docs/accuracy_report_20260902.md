...
現在の主指標は、8 fixture・287採点項目に対して **260/287 = 90.6%**。
...
- 法人番号は記号除去後13桁だけを採用し、12桁を補完しない。不確かな値はprimaryへ残さず、候補として保持して人に回す。
- 性別・婚姻状態は内部値へ統一し、引用は原文のまま残す。
- 役職名、現在職、採用後職種を分け、職種は既知の完全一致aliasだけ正式RASENSラベルへ変換する。
