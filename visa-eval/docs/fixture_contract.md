...
- `eval_runs/<run_id>/<case_id>/run_manifest.json`: 抽出runの再現性メタデータ。model、thinking設定、retry設定、コードcommitとdirty状態、prompt/schema hash、input hash、入力document count、入力bytes合計を記録する。PII値、実ケース識別子、実ファイル名は入れない。golden revisionはブラインド抽出runには入れず、比較結果側で記録する。
- mismatch確認用の `report.html`、証拠画像、レンダリング済みページ画像は restricted 生成物として扱う。保存する場合は `eval_runs/`、`blind_runs_from_test_cases/**/generated/`、`qa/` などgitignore配下に限定し、git管理対象へ置かない。
