...
このディレクトリでは、実データ利用を許可します。ただし、`raw/`、`test_cases_from_raw/**`、`generated/**`、レビュー用キャプチャ、DevTools出力はすべて `restricted test data` として扱います。
...
- 管理する: `README.md`, `eval_config/suites/*.json`, `test_cases_from_raw/README.md`, `docs/`
- 管理しない: `raw/`, `archived/`, `test_cases_from_raw/<case_id>/`, `generated/`
...
| DevTools console / screenshot / generated JSON 残存 | `test_cases_from_raw/<case_id>/<applicant_id>/generated/` やDevTools出力に氏名、住所、学歴、雇用条件、自由記述が残り、再利用時に漏えいする。 | `generated/` は期待値比較に必要な最小限だけ保持する。Console実値ログを避け、スクリーンショットや生成JSONを共有する場合はマスク版を使う。 |
