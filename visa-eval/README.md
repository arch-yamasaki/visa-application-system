...
このディレクトリでは、実データ利用を許可します。ただし、`raw/`、`catalog.json`、`test_cases_from_raw/**`、`generated/**`、レビュー用キャプチャ、DevTools出力はすべて `restricted test data` として扱います。
...
- 管理しない: `raw/`, `catalog.json`, `test_cases_from_raw/<case_id>/`, `generated/`

`catalog.json` や `test_cases_from_raw/*/*/expected/*.golden.json` はJSONでも実PIIを含むため、ソースコードや設計資産とは分けて扱う。
