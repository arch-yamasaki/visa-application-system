...
- 管理する: `README.md`, `eval_config/suites/*.json`, `test_cases_from_raw/README.md`
- 管理しない: `raw/`, `test_cases_from_raw/<case_id>/`, `generated/`
...
旧一括再分類フローは廃止したため、raw配下を更新しても自動メタデータ生成は行いません。単票fixtureの追加・更新は、既存の `test_cases_from_raw/<case_id>/<applicant_id>/` を参考に `scenario.json` と `input/document_manifest.json` を個別に整備します。
...
当面の評価スイートは `eval_config/suites/single_smoke.json` のみを使う。参照先は `test_cases_from_raw/<base_case_id>/<applicant_id>/` で、1申請人=1フォームの入力生成を評価する。
