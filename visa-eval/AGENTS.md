...
今後のGemini bytes evalは、各run出力に `run_manifest.json` を含める。記録対象は model、thinking設定、retry設定、コードcommitとdirty状態、prompt/schema hash、input hash、入力document count、入力bytes合計など、抽出runを再現するための非PIIメタデータに限る。PII値、実ケース識別子、実ファイル名はmanifestへ入れない。golden revisionはブラインド抽出runのmanifestには入れず、`compare_with_golden.py` の比較結果側で記録する。
