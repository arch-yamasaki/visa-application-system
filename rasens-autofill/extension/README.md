...
6. 案件一覧 `select` から対象案件を選び、「visa-appから読込」を実行
   - API URL未設定時は `https://visa-app-913363513517.asia-northeast1.run.app` を使います。
   - ローカルに接続する場合は「接続先設定」で `http://localhost:8080` に上書きします。
   - 案件一覧が取得できない場合や一覧外の案件を使う場合は、`case_id` を手入力してください。
   - 一覧を再取得したい場合は「更新」を押してください。
7. 「入力対象を確認」で件数を確認
8. 「一括入力」または「ゆっくり入力」を実行
...
同梱JSONの `confidence` は現状 `demo` または `generated` が中心です。OCR不確実性やPDF根拠の弱さはまだ十分に表現できていないため、実案件データを投入する場合は元資料と照合してください。
