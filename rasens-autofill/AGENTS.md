...
rasens-autofill/
  data/
    schemas/           # 評価補助用の document_manifest, review JSON Schema
...
- `data/schemas/*.json`: AIレビュー出力と入力資料manifestの評価補助用 JSON Schema
...
Chrome拡張は Cloud Run の visa-app API を固定で使う。

- Cloud Run: `https://visa-app-913363513517.asia-northeast1.run.app`

ポップアップUIには `case_id` 手入力、同梱データ読込、接続先設定を置かない。案件一覧から対象案件を選び、`/cases/{case_id}/application-data` の rows を取得して RASENS DOM へ入力する。
