...
> **補足**: 実際にはAIに聞く前に、無料の決定的な位置特定（anchor resolver）を先に試す。
> PDFのテキスト層・XLSXセル・DOCXブロックから quote を検索し、一意に見つかればAI呼び出し不要で確定する。
> 複数箇所で見つかった場合は候補位置（最大3件）を保存してレビュアが選ぶ。
> 以下のAIによる座標推定は「テキスト層で解決できなかった対象フィールド」だけに使うfallback。
