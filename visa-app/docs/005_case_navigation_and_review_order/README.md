...
状態: 完了。`/cases` は `case_data` から生成した `display_name`, `applicant_name`, `employer_name`, `target_status`, `application_type`, `updated_at` を返す。`display_name` は保存せず、一覧用DTOとして都度生成する。
...
状態: 完了。案件一覧では `display_name` を主表示にし、申請人、所属機関、在留資格、状態を見せる。`case_id` はコピー可能な補助情報として残す。
...
状態: 完了。拡張popupは `/cases` を取得して案件を選択できる。`case_id` 手入力欄は一覧取得失敗時や一覧外案件のfallbackとして残す。
