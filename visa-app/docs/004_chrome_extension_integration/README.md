...
### 1. `ready_to_fill` 依存が残っている

`application_data.py` は `workflow_state == "ready_to_fill"` のときだけ `fillable=true` にする。

`api_client.js` の case 一覧取得も `workflow_state=ready_to_fill` 固定になっている。

frontend では `ready_to_fill` への遷移をやめたため、このままだと「レビュー画面では保存できたが、拡張では投入不可」と見える。

### 2. 拡張側の状態表示が古い

`popup.js` は `要レビュー`、`入力準備完了` という表示をまだ持っている。

visa-app frontend と同じく、拡張側も `未抽出 / 抽出中 / 抽出済み / 抽出失敗` に寄せる。

### 3. 実RASENS画面での field_id / field_name 適合率が未確定
...
`application_data.py` の投入可能状態を広げる。
...
`popup.js` に frontend と同じ表示正規化を入れる。
...
最小案は、拡張の主導線を case ID 指定にして、一覧取得は一旦補助扱いにすること。

一覧を残す場合は、backend 側に `workflow_state=extracted` 相当の検索を用意するか、複数状態を取得して拡張側でまとめる。
