...
2026-09-02時点の現在主指標は、[`visa-eval/docs/accuracy_report_20260902.md`](../../../visa-eval/docs/accuracy_report_20260902.md) の 8 fixture・287 verified extraction値項目に対する **260/287 = 90.6%** です。これはモデル改善ではなく、住所・法人番号・役職のgolden契約を再監査し、採点scopeを修正した影響です。以前の 315項目に対する **259/315 = 82.2%** は、golden再監査前の履歴値として扱います。以下の初期fixtureに関する数値は、評価方法を整備する前の履歴説明です。
...
| 抽出比較 | Gemini出力が現在のgoldenと合うか | `eval_runs/<run_id>/<case_id>` vs `expected` | 暫定のみ |
...
現在のレポートでは、少なくとも次を分けて読む。

| 指標 | 意味 | 分母への入れ方 |
|---|---|---|
| 値精度 | AI `case_data` が verified extraction のgolden値と一致するか | 主指標の分母 |
| source_ref充足 | 値に `source_refs`、quote、input manifest内document id があるか | 値精度とは別集計 |
| anchor表示 | PDF bbox / XLSX cell / DOCX block として画面で移動・ハイライトできるか | 値精度とは別集計 |
| semantic正確性 | anchorが意味上もその値の根拠箇所を指しているか | 人手レビュー指標 |
| extra | AIには非空値があるがgoldenに同pathがない項目 | 正答率の分母・分子へ入れない |
...
### 4. RASENS出力と添付資料ページを分ける

提出済み申請書PDFには、RASENS入力済み申請書ページと、旅券・証明書などの添付資料ページが混ざることがあります。
この場合は、fixture作成時に物理分割します。
...
`output/` にだけ存在し、Gemini入力資料に存在しない値は、Geminiの抽出ミスとして扱いません。
...
3. raw資料を見て case_data.golden.json を補完・確認する
   |
   v
4. Gemini bytes eval を eval_runs/<run_id>/<case_id> に出す
   |
   v
...
- `output/rasens_application/` を最終提出値の一次監査資料として確認する。
- `input/` の資料は、その値がGemini入力資料から抽出可能だったかを判断するために使う。
- `output/` にしかない値は、Gemini抽出ミスではなく固定値・手入力値・入力資料不足として分類する。
...
ここで入れるのは、一般的な文字列類似度による fuzzy matching ではありません。
path / field type を見て、比較時だけ値を canonicalize します。

例:

- boolean-like field: `No` / `無` / `False` / `なし` は同値。
- country-like field: `NEPAL` / `Nepal` / `ネパール Nepal` は同値。
- enum label: RASENSの同じ選択肢を指す `雇用 Employment` / `雇用` は同値。
- amount/date/phone: カンマ、円表記、区切り文字だけの差は同値。

一方で、次は不一致のまま残します。

- `Married` と `Single`
- `Nepal` と `India`
- `2026-03-01` と `2026-04-01`
- `260000` と `360000`

正規化で変えるのは `match` / `mismatch` の判定だけです。
`golden_total` は変えず、主指標は引き続き `一致 / golden側の比較対象項目数` にします。
...
- restricted raw、eval run output、expected golden を git に入れる。
