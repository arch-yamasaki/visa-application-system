...
- [x] `canonical_case_data_v2.md` と `frontend/src/types/caseData.ts` を canonical v2 に更新する。
...
| 旧autofill側のcase_data JSON Schema | 旧autofill寄りschema | 現行仕様とズレていたため削除 |
| `scripts/build_application_data.py` | case_data + mapping -> rows のCLI | `visa-eval/scripts/build_application_data.py` へ移動し、backend generatorを呼ぶ評価用CLIに変更 |
...
|---|---|---|
| Data Contract Agent | `canonical_case_data_v2.md`, `caseData.ts`, backend schema | canonical v2 path / type / required方針の確定 |
...
- `build_application_data.py` は `visa-eval/scripts/` に移動し、backend generator を呼ぶCLIとして扱う。
