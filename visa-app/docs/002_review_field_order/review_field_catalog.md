...
`rasens_offer_mapping.json` は canonical v2 に合わせて作り直し対象です。現行55 mappingを `rasens_offer_fields.json` の `field_id` / `field_name` と突合すると、22件で `field_id` 不在、`field_name` 不在、または `field_id` と `field_name` が別項目を指す疑いがあります。

特に所属機関まわりは危険です。例として、`employer.address` は `field_id` では所在地を指しますが、`field_name` は高度専門職ポイント表の「部分合計」側を指している疑いがあります。Chrome拡張が `field_name` 優先で投入すると別項目に入るため、backend generator 実装時にフォーム台帳から mapping を再生成します。
...
| フォーム順 | 画面番号（参考） | RASENS表示名 | visa-app表示 | canonical path | 入力方針 | application mapping | 備考 |
|---:|---|---|---|---|---|---|---|
| 54 | 取次者 | 氏名 | 非表示 | `intermediary.name` | 設定値 | 予定 | 太田さん側の申請アカウントを持つ申請会社情報から注入 |
| 55 | 取次者 | 郵便番号 | 非表示 | `intermediary.postal_code` | 設定値 | 予定 | 案件ごとの抽出対象ではない |
| 56 | 取次者 | 住所 | 非表示 | `intermediary.address` | 設定値 | 予定 | 案件ごとの抽出対象ではない |
| 57 | 取次者 | 所属機関等 | 非表示 | `intermediary.organization` | 設定値 | 予定 | 申請アカウントを持つ申請会社名 |
| 58 | 取次者 | 電話番号 | 非表示 | `intermediary.phone` | 設定値 | 予定 | 案件ごとの抽出対象ではない |
...
`rasens_offer_mapping.json` と backend generator は次を満たすようにします。
