...
| 5 | 取次者 | 5 | 非表示 / 設定値 | `intermediary`。Firestore `org_settings/{org_id}` の組織設定。案件ごとの抽出対象ではない |
| 6 | 所属機関に関する情報等 | 54 | 表示 | 雇用会社、契約、給与、職務内容 |
| 7 | 高度専門職ポイント表 | 57 | 非対応 | 技人国MVPでは対象外 |
| 8 | 受領方法等 | 3 | 非表示 / 設定値 | `org_settings/{org_id}` の通知メールを使い、受領方法は `メール Email` 固定 |
...
| 受領方法等 | 3 / 3 | 組織設定の通知メールと、そこから生成する再入力欄に対応済み |
...
| フォーム順 | 画面番号（参考） | RASENS表示名 | visa-app表示 | canonical path | 入力方針 | application mapping | 備考 |
|---:|---|---|---|---|---|---|---|
| 54 | 取次者 | 氏名 | 表示（固定・編集不可） | `settings.intermediary.name` | 設定値 | あり | `org_settings/{org_id}` から注入 |
...
| 受領方法等 | 設定値 | `org_settings/{org_id}` の `receiving_method` から注入 |
