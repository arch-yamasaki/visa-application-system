...
| 種類 | 保存・管理場所 | 用途 |
|---|---|---|
| schema required | `case_data.schema.json` / backend・frontend型 | `case_data` の構造を保つための最小必須項目 |
...
| `employment_conditions.employment_period_type` | `employment.period_type` |
| `employment_conditions.employment_period_years` | `employment.period_years` |
| `employment_conditions.employment_period_months` | `employment.period_months` |
