...
- Firestore の `workflow_state` が `extracted` になる。
...
partial extraction は失敗扱いにしません。成功scopeの結果は人間レビューで確認できるように `extracted` として保存します。失敗scopeは `review.validation_errors` に明示します。全主要scopeが失敗した場合だけ `failed` とします。
...
| レビュー画面の入口 | `workflow_state=extracted` |
...
| `workflow_state` | `extracted` か `failed` か |
...
| state | `extracted` | レビュー画面で確認・編集できる |
| fillable | `true` | Chrome拡張でpreview / 入力できる |

この状態は「失敗」ではなく「レビュー可能な途中結果」です。全主要scopeが失敗した場合だけ `failed` として扱います。
...
`/application-data` は `extracted` 相当のケースを自動入力可能にします。
