...
5. 実データQA後に旧 `source` 文字列の互換処理を削除する。
6. 実データで Gemini schema error が出ないか確認する。
7. 旧 raw `source_refs[]` と旧 `field_metadata` 別出しレスポンス互換を削除する。
...
| Engineer | Gemini raw response は `{ value, source_ref }` のみを受ける |
...
- `backend/extractors/gemini.py`
  ...
  - 旧 `{ value, source }` 文字列互換は削除
  - 旧 raw `{ value, source_refs }` レスポンスは拒否
  - 旧 `field_metadata` 別出しレスポンス互換は削除
...
- `field_metadata.source_refs[]` は保存・表示用の現行形式であり、Gemini raw response に直接出させる形式ではない。
