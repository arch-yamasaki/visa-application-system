...
| レイヤー | 比較対象 | 目的 | 自動比較 |
|---|---|---|---|
| A. 抽出値 | `case_data.golden.json` | 申請データの値が期待通りか見る | 初期MVPで実施 |
| B. レビュー指摘 | `review.golden.json` | 不足・矛盾・要確認が期待通りか見る | 後回し |
| C. 証跡 | `field_metadata.golden.json` | `document_id`, `page`, `text_quote`, `anchor`, `bbox` を見る | 後回し |
| D. 入力データ | `case_data` から都度生成 | Chrome拡張へ渡す rows が期待通りか見る | 初期MVPで実施 |

最初から bbox の正解率を厳密に採点しない。bbox は人間の見え方に依存し、正解領域の作成コストも高い。

初期は `case_data` と、そこから都度生成する `application_data` の比較を主にする。`application_data.golden.json` は派生物なので初期MVPでは正本にしない。`field_metadata` は「証跡レビュー用の成果物」として残し、`source_ref` と anchor 構造が安定した後に比較ロジックを追加する。
...
visa-eval/test_cases_from_raw/<base_case_id>/<applicant_id>/
  ...
  expected/
    ...
    review.golden.json             # 残すが初期MVPではgateにしない
...
初期MVPでは作らない。generated の `field_metadata.json` を人間が見るだけにする。
...
AIに手書きさせない。初期MVPでは `application_data.golden.json` を読まず、expected / generated の `case_data` から backend generator でその場生成して比較する。
...
`field_id`, `field_name`, `input_type` は後で比較対象に加える。最初から広げると、抽出値ではなくmapping表記差分でノイズが増える。
...
- [x] `compare_with_golden.py` は当面 `case_data`, `application_data` の比較をデフォルトにする
- [x] `review` は `--targets review` を指定した時だけ比較する
- [x] `field_metadata` 比較は未実装のまま、将来TODOとして残す
- [x] `application_data` は保存済みgoldenではなく、`case_data` から都度生成して比較する

### Step 4. Phase 1 へ拡張

- [x] 3ケースに増やす
- [x] 同一会社書類で applicant を取り違えないか確認する対象を含める
- [x] 家族在住・不足確認パターンを含める
- [x] fixtureごとの `comparison_report.md` を残す
...
| bboxの正解 | 初期は自動採点しない |
| CI | 実PIIは載せない |
| `application_data.golden.json` | 初期MVPでは正本にしない。`case_data` から比較時生成 |
