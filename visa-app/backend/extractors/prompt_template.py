...
- `employment.job_category_primary`: 申請先で従事予定の業務について、RASENSの職種区分ラベルを正確に入れること。現fixtureで必要な建築系は `建築・土木・測量技術 Architecture, civil engineering, surveying techniques`、管理業務系は `管理業務（経営者を除く） Management work (excluding executives)` とする。申請人の現在職業 `applicant.occupation` と混ぜないこと。
...
- 職種（`employment.job_category_primary`）: 予定業務のRASENS職種区分を正確な選択肢ラベルで出力すること。建築・土木・測量系は `建築・土木・測量技術 Architecture, civil engineering, surveying techniques`、管理業務系は `管理業務（経営者を除く） Management work (excluding executives)` を使う。申請人の現在職業 `applicant.occupation` とは別物として扱うこと。
