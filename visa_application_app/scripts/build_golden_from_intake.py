#!/usr/bin/env python3
"""Build restricted golden case_data from intake spreadsheets.
...
def main() -> None:
    ...
    for case in catalog["cases"]:
        ...
        (expected / "case_data.golden.json").write_text(json.dumps(case_data, ensure_ascii=False, indent=2) + "\n")
        (expected / "review.golden.json").write_text(json.dumps(build_review(case_data), ensure_ascii=False, indent=2) + "\n")
        # Batch cases do not map cleanly to one online form; keep explicit empty golden until per-applicant fixtures are split.
        (expected / "application_data.golden.json").write_text("[]\n")
        print(f"wrote {case['case_id']} with {len(case_data['applicants'])} applicants")
